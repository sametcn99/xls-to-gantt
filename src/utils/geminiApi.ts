 ...
 */
export async function standardizeDatesWithGemini(
  dateValues: Array<string | number | Date>,
): Promise<string[]> {
  try {
    // Skip processing if API key is not set
    if (!API_KEY) {
      console.error(
        "Gemini API key not set. Please set NEXT_PUBLIC_GEMINI_API_KEY environment variable.",
      );
